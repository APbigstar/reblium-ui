...
app.post('/api/add_avatar', (req, res) => {
  ...
  const addAvatarQuery = 'INSERT INTO Avatar (Avatar_Name, User_Info_id, Avatar_Image) VALUES (?, ?, ?)';
  connection.query(addAvatarQuery, [avatarName, user_info_id, avatarImage], (err, results) => {
    if (err) {
      ...
      console.log('Avatar name added to the database:', avatarName, 'with ID:', user_info_id);
...
app.post('/api/add_user', (req, res) => {
  ...
  connection.query(checkUserExistsQuery, [user_id], (err, results) => {
    if (err) {
      console.error('Error checking if User_id exists:', err);
      return res.status(500).json({ error: 'Error checking if User_id exists' });
    } else {
      if (results.length === 0) {
        ...
        connection.query(addUserQuery, [user_id], (err, results) => {
          if (err) {
            ...
            const user_info_id = results.insertId;
            console.log('New user added to the database with ID:', user_info_id);
