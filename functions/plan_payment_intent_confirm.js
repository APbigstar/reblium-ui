...
const verifyXsollaToken = async (req, res, next) => {
  ...
  try {
    const response = await axios.post(`https://login.xsolla.com/api/users/${projectId}/user/info`, null, {
      headers: {
...
router.post('/', verifyXsollaToken, async (req, res) => {
  ...
  try {
    ...
    if (userPlans.length === 0) {
      return res.status(500).json({ success: false, error: 'Error fetching user plan' });
    }
    ...
    if (stripeCustomer.length === 0) {
      return res.status(500).json({ success: false, error: 'Error fetching stripe customer' });
    }

    const customer_id = stripeCustomer[0].customer_id;

    if (userPlans[0] && payment_intent.customer === customer_id && payment_intent.status === 'succeeded') {
